...
export default createWidget('nav-map', {
  ...
  html(attrs, state) {
    let category = state.category = attrs.category;
    ...
    setupMap(category, mapObjs.map, mapObjs.geojson, state.showAttribution)
    ...
    return [
      ...
      this.editCategoryBtn(attrs.isTopic, category),
      new RawHtml({ html: mapObjs.element }),
      this.aboutList(category, attrs.isHome),
      this.attach('button', {
